...
public class LoginUtil {
    ...
    private static final String LOGIN_FAILED_CAUSE_PASS_WRONG =
    ...
    private static boolean IS_IBM_JDK;

    static {
        String s = System.getProperty("java.vendor");
        IS_IBM_JDK = s != null && s.contains("IBM");
    }

    private final static Object lockObject = new Object();

    public static void checkLogin(Config config) {
        if (null == config) {
            LOG.error("config is null when check login");
            System.exit(0);
        }
        if (!config.getBooleanConfig(Constant.NEED_LOGIN)) {
            return;
        }
        try {
            LoginUtil.setJaasConf(config.getStringConfig(Constant.ZK_CLIENT_NAME),
                    config.getStringConfig(Constant.USER_PRINCIPAL),
                    config.getStringConfig(Constant.USER_KEYTAB_PATH));
            LoginUtil.setZookeeperServerPrincipal(config.getStringConfig(Constant.ZK_SERVER_PRINCIPAL_KEY),
                    config.getStringConfig(Constant.ZK_SERVER_PRINCIPAL));
            LoginUtil.login(config.getStringConfig(Constant.USER_PRINCIPAL), config.getStringConfig(Constant.USER_KEYTAB_PATH),
                    config.getStringConfig(Constant.KRB5_CONF_PATH), new Configuration());
        }catch (IOException e) {
            LOG.error("IOException--login failed");
        }
    }

    public static void login(String userPrincipal, String userKeytabPath, String krb5ConfPath, Configuration conf)
            throws IOException
    {
...
//    }
            ...
            throws IOException
    {
        ...
        for (AppConfigurationEntry entry : entrys) {
            String tmp = (String) entry.getOptions().get("principal");
            if (tmp.equals(principal)) {
                checkPrincipal = true;
            }

            if (IS_IBM_JDK) {
                tmp = (String) entry.getOptions().get("useKeytab");
                if (tmp.equals(keytabFile))
                {
                    checkKeytab = true;
                }
            }
            else {
                tmp = (String) entry.getOptions().get("keyTab");
                if (tmp.equals(keytabFile)) {
                    checkKeytab = true;
                }
            }
            ...
            throws IOException
    {
        ...
        catch (IOException e)
        {
            ...
            LOG.error("perhaps cause 1 is " + LOGIN_FAILED_CAUSE_PASS_WRONG + ".");
...
//    }
    ...
    private static class JaasConfiguration extends javax.security.auth.login.Configuration
    {
        ...
        public AppConfigurationEntry[] getAppConfigurationEntry(String appName) {
            ...
            return baseConfig == null ? new AppConfigurationEntry[]{} : baseConfig.getAppConfigurationEntry(appName);
        }
    }
}